...
/*
 * The priority queue/heap based approach hash a complexity O(n) when using an
 * efficient heap construction algorithm (heapify such as Floyd's Build-Heap).
 * The lookup of the top element then happens in constant time.
 *
 * For reasonably small arrays, the sorting algorithm will still perform better
 * despite its O(n log(n)) complexity.
 */
class Solution {
  public:
    ...
    std::vector<std::string> findRelativeRanksSorting(std::vector<int> &score) {
        std::vector<std::size_t> indices(score.size());
        std::iota(indices.begin(), indices.end(), 0);
