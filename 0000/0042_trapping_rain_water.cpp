...
class Solution {
  public:
    int trap(const std::vector<int> &height) {
        ...
        // find the maximum height up to and including column i from the left
        for (std::size_t i = 1; i < n; ++i) {
            max_height_to_left[i] =
                std::max(max_height_to_left[i - 1], height[i]);
        }

        // find the maximum height up to and including column i from the right
        for (int i = n - 2; i >= 0; --i) {
        ...
        for (std::size_t i = 0; i < n; ++i) {
            ...
            // Firstly, find the minimum of the maximum height to the left and
            // right, the water cannot be higher than that. Secondly, subtract
            // the height of the column itself.
            trapped_water +=
