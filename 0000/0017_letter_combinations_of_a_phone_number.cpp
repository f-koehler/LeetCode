...
class Solution {
  private:
    ...
    void build_all_combinations(const std::vector<std::size_t> &digits,
        ...
        // check if we have met the target length -> found a valid combination
  ...
  public:
    std::vector<std::string> letterCombinations(const std::string &digits) {
        ...
        build_all_combinations(digits_int, current_combination, combinations);
...
TEST_CASE("0017 - Letter Combinations of a Phone Number",
    ...
    SECTION("Example 1") {
        ...
        REQUIRE(result == std::vector<std::string>{"ad", "ae", "af", "bd", "be",
                                                   "bf", "cd", "ce", "cf"});
    }
    SECTION("Example 2") {
