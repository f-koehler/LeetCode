...
namespace P0392 {
     ...
     * positions for each character. Since we will have a single lookup table
     * per character we can store them in an unordered map for efficient lookup
     * O(1). The lookup table for each character is then a std::set (i.e.
     * ordered). This allows us to check for an occurence of a character after
     * the current index in the t string in O(log(n)).
     */
      ...
      public:
        explicit IsSubsequenceCache(const std::string &t) {
            ...
            for (std::size_t i = 0; i < n; ++i) {
                ...
                if (auto pos = m_positions.find(c); pos != m_positions.end()) {
                    ...
                    // consideration -> insert it using end() as a hint since
                    // the new index should be the largest one
                    pos->second.insert(pos->second.end(), i);
                    continue;
                }

                // we found a new character and create a new position set for
                // its
        ...
        bool operator()(const std::string &s) const {
            ...
            for (const char c : s) {
                ...
                // check if there is an occurence of c after the current index
                // in t
                const auto earliest_c =
                    c_positions->second.lower_bound(index + 1);
    ...
    class Solution {
      public:
        ...
        bool isSubsequenceCached(const std::string &s, const std::string &t) {
            // Of course this only really makes sense we check a lot of s's
            // agains the same t. This function is just to use the solution more
            // easily on LeetCode's platform.
