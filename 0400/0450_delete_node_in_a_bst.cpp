    ...
    class Solution {
      public:
        TreeNode *deleteNode(TreeNode *root, int key) {
            ...
            if (key < root->val) {
                // key must be in left subtree, delete it there and set its root
                // as left child of current node
                root->left = deleteNode(root->left, key);
                return root;
            }
            if (key > root->val) {
                // key must be in right subtree, delete it there and set its
                // root as right child of current node
            ...
            // find the smallest node in the right subtree, remove it from there
            // and make it the new root with the original left subtree as the
            // left childe and the remaining right subtree as the right child
