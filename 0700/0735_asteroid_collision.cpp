    ...
    class Solution {
      public:
        std::vector<int> asteroidCollision(const std::vector<int> &asteroids) {
            ...
            // However, we need to return a vector so we use that. There should
            // be no performance downside to this choice if we reserve the
            // maximal possible size of a stable row.
            ...
            for (const int &asteroid : asteroids) {
                ...
                while (true) {
                    // The next asteroid does not cause a collision if one of
                    ...
                    // The asteroid is moving to the left and the last asteroid
                    // in the stable row to the right -> collision.
                    ...
                    if (size < stable_row.back()) {
                        // The new asteroid is destroyed by the last asteroid in
                        // the row.
                        break;
                    }
